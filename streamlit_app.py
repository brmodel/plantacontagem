...
    # Removido: logging.info(f"Tentando carregar dados de: {url}")
    try:
        # Alteração crucial: Usa essential_cols (lista de nomes) para usecols # Traduzido
...
            # Removido: logging.warning(f"{rows_before - rows_after} linhas removidas devido a valores inválidos em Numeral, lat ou lon.")
            st.warning(f"{rows_before - rows_after} linhas removidas devido a valores inválidos em Numeral, lat ou lon.") # Convertido para st.warning
...
        # Removido: logging.error(f"Erro de rede ao buscar dados: {e}")
    except pd.errors.EmptyDataError:
        st.error("O arquivo CSV da planilha parece estar vazio ou não contém cabeçalhos.")
        # Removido: logging.error("EmptyDataError: CSV is empty or headerless.")
...
        # Removido: logging.error(f"ValueError durante o parsing ou usecols: {e}")
    except Exception as e:
        st.error(f"Erro inesperado ao carregar dados: {e}")
        # Removido: logging.exception("Erro inesperado no load_data")
        return pd.DataFrame()
...
            # Removido: logging.warning("Estrutura do GeoJSON inválida.")
...
        # Removido: logging.error("Timeout ao carregar GeoJSON.")
    except requests.exceptions.RequestException as e:
        st.error(f"Erro de rede ao carregar GeoJSON: {e}")
        # Removido: logging.error(f"Erro de rede GeoJSON: {e}")
    except ValueError as e:
        st.error(f"Erro ao decodificar GeoJSON: {e}")
        # Removido: logging.error(f"Erro de decodificação JSON: {e}")
    except Exception as e:
        st.error(f"Erro inesperado ao carregar GeoJSON: {e}")
        # Removido: logging.exception("Erro inesperado no load_geojson")
    return default_geojson
...
                    # Removido: logging.warning(f"ID da regional '{region_id}' não é numérico. Ignorando na legenda.")
                    st.warning(f"ID da regional '{region_id}' não é numérico. Ignorando na legenda.") # Convertido para st.warning
...
    else:
        # Removido: logging.warning("Dados GeoJSON não disponíveis ou vazios para adicionar ao mapa.")
        pass # Não exibe warning se GeoJSON não carregar aqui, já é mostrado na função de carregamento
...
                # Removido: logging.warning(f"Coordenadas inválidas ou ausentes para {row.get('Nome', 'N/I')} (Índice: {index}). Pulando marcador.")
                st.warning(f"Coordenadas inválidas ou ausentes para {row.get('Nome', 'N/I')} (Índice: {index}). Pulando marcador.") # Convertido para st.warning
                continue
...
            except Exception as e:
                # Removido: logging.error(f"Erro ao carregar ícone {icon_url} para {row.get('Nome', 'N/I')}: {e}. Usando ícone padrão.")
                st.error(f"Erro ao carregar ícone {icon_url} para {row.get('Nome', 'N/I')}: {e}. Usando ícone padrão.") # Convertido para st.error
...
            else:
                # Removido: logging.warning("GeoJSON não carregado ou inválido.")
...
        # A chave "search_input_widget" mantém o estado do input. # Traduzido
...
        # Só atualiza o estado da busca se o valor do widget mudou # Traduzido
        if search_query != st.session_state.search_input_value:
            st.session_state.search_input_value = search_query
            # Opcional: Se a busca mudar, talvez queira limpar a info da sidebar # Traduzido
...
        # Removido: logging.info(f"Renderizando mapa com {len(df_filtrado)} unidades filtradas.")
